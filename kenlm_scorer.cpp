...
#include "ctcdecode/src/decoder_utils.h"
#include <iomanip>
...
void Kenlm_Scorer::load_lm(const std::string& lm_path) {
...
double Kenlm_Scorer::get_log_cond_prob(const std::vector<std::string>& words) {
...
double Kenlm_Scorer::get_sent_log_prob(const std::vector<std::string>& words) {
...
double Kenlm_Scorer::get_log_prob(const std::vector<std::string>& words) {
...
void Kenlm_Scorer::reset_params(float alpha, float beta) {
  this->alpha = alpha;
  this->beta = beta;
}

std::string Kenlm_Scorer::vec2str(const std::vector<int>& input) {
  std::string word;
  for (auto ind : input) {
    word += char_list_[ind];
  }
...
std::vector<std::string> Kenlm_Scorer::split_labels(const std::vector<int>& labels) {
...
void Kenlm_Scorer::set_char_map(const std::vector<std::string>& char_list) {
...
std::vector<std::string> Kenlm_Scorer::make_ngram(PathTrie* prefix) {
...
void Kenlm_Scorer::fill_dictionary(bool add_space) {
